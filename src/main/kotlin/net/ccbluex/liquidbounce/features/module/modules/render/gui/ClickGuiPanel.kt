...
class ClickGuiPanel(
    ...
    fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
        ...
        return if (!isClickWithinBounds(intMouseX, intMouseY)) {
            // If clicking outside panel bounds, clean up all active states
            cleanupActiveStates()
            false
        } else {
            when {
                ...
                isModuleAreaClick(intMouseY) -> {
                    // Check if click is outside any widget but inside module area
                    if (!isClickOnModuleOrWidget(intMouseX, intMouseY)) {
                        // If click is in module area but not on a module or widget, clean up active states
                        cleanupActiveStates()
                    }
    ...
    private fun isClickOnModuleOrWidget(mouseX: Int, mouseY: Int): Boolean {
        var currentY = y + headerHeight - scrollOffset.toInt()

        for (module in filteredModules) {
            if (isClickOnSpecificModule(module, mouseX, mouseY, currentY)) {
                return true
            }
            currentY = updateYPosition(module, currentY)
        }

        return false // Click is neither on a module nor a widget
    }

    private fun isClickOnSpecificModule(module: ClientModule, mouseX: Int, mouseY: Int, currentY: Int): Boolean {
        ...
        if (mouseY >= currentY && mouseY < moduleBottomY) {
            return true // Click is on a module button
        }

        return isClickOnModuleWidgets(module, mouseX, mouseY, moduleBottomY)
    }

    private fun isClickOnModuleWidgets(module: ClientModule, mouseX: Int, mouseY: Int, startY: Int): Boolean {
        if (!expandedModules.getOrDefault(module, false)) {
            return false
        }

        val widgets = moduleSettingWidgets[module] ?: return false
        var currentY = startY
        ...
        for (widget in widgets) {
            ...
            if (widget.isMouseOver(mouseX, mouseY)) {
                return true // Click is on a widget
            }
            currentY += SETTING_HEIGHT + SETTING_SPACING
        }
    ...
    private fun updateYPosition(module: ClientModule, currentY: Int): Int {
        val moduleBottomY = currentY + moduleHeight
        
        return if (expandedModules.getOrDefault(module, false)) {
            val widgets = moduleSettingWidgets[module] ?: return moduleBottomY
            moduleBottomY + widgets.size * (SETTING_HEIGHT + SETTING_SPACING)
        } else {
            moduleBottomY + (moduleSettingWidgets[module]?.size ?: 0) * (SETTING_HEIGHT + SETTING_SPACING)
        }
    }
    ...
    @Suppress("UnusedParameter", "detekt:CognitiveComplexMethod", "detekt:NestedBlockDepth")
    ...
    /**
     * Clean up active states of all widgets to prevent crashes when user interacts
     * with settings and then clicks elsewhere
     */
    private fun cleanupActiveStates() {
        // Use the new cleanupActiveState method that we added to each widget
        moduleSettingWidgets.values.flatten().forEach { widget ->
            widget.cleanupActiveState()
        }
    }
    ...
    /**
     * Dispose of all widgets in this panel to prevent memory leaks
     */
    fun dispose() {
        for (widgets in moduleSettingWidgets.values) {
            for (widget in widgets) {
                widget.dispose()
            }
        }
        moduleSettingWidgets.clear()
        openDropdown = null
    }
