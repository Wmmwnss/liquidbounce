...
abstract class SettingWidget<T>(
    ...
    open fun dispose() {} // Method to clean up resources and listeners
    
    /**
     * Clean up active interaction states (editing, dragging, etc.) without fully disposing the widget
     */
    open fun cleanupActiveState() {}
...
class BooleanSettingWidget(
    ...
    // Flag to determine if the widget is still active
    ...
    override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
        if (!isActive || button != 0 || !isMouseOver(mouseX.toInt(), mouseY.toInt())) return false
    ...
    override fun dispose() {
        ...
        // Note: We can't directly nullify onValueChanged in Kotlin, 
        // but setting isActive to false prevents further execution
    }
}
...
class FloatSettingWidget(
    ...
    fun mouseDragged(mouseX: Double, mouseY: Double, button: Int): Boolean {
        if (!isActive || !isDragging || button != 0) return false
...
class TextSettingWidget(
    ...
    override fun cleanupActiveState() {
        if (isEditing) {
            // Update the actual value if user was editing without pressing Enter
            value = editingValue
            isEditing = false
        }
    }
...
class IntRangeSliderWidget(
    ...
    @Suppress("UnusedParameter")
    fun mouseDragged(mouseX: Double, mouseY: Double, button: Int): Boolean {
        if (!isActive || !(isDraggingStart || isDraggingEnd) || button != 0) return false
...
class EnumSettingWidget(
    ...
    override fun keyPressed(keyCode: Int, scanCode: Int, modifiers: Int): Boolean {
        if (!isActive || !isDropdownOpen) return false
