...
class ClickGuiScreen : Screen(Text.literal("ClickGUI")) {
    ...
    private fun initializeManagers() {
        // Reload persisted panel and accordion states from disk
    ...
    private fun rebuildPanels() {
        ...
        for ((category, modules) in modulesByCategory) {
            ...
            val panel = ClickGuiPanel(
                ...
                allModules = modules,
                config = panelConfig,
                onPanelStateChanged = { updatedCategory, x, y, expanded ->
                    ClickGuiConfigManager.savePanelState(updatedCategory, x, y, expanded)
                }
            )
    ...
    override fun close() {
        // Dispose of all panels to clean up listeners and prevent memory leaks
        for (panel in panels.values) {
            panel.dispose()
        }
        mc.mouse.lockCursor()
