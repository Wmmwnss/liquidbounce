...
object CommandManager : Collection<Command> by commandSet {
     ...
     */
    private fun getSubCommand(args: List<String>): Pair<Command, Int>? {
        if (args.isEmpty()) return null
        var currentCommand = rootCommandMap[args[0]] ?: return null
        var idx = 0
        for (i in 1 until args.size) {
            val sub = currentCommand.subcommandMap[args[i]] ?: break
            currentCommand = sub
            idx = i
        }
        return Pair(currentCommand, idx)
    }
    ...
    val pair = validateAndGetCommand(args)
    val command = pair.first
    val idx = pair.second

    validateCommandParameters(command, args, idx)
    val parsedParameters = parseCommandParameters(command, args, idx)
    ...
    private fun validateAndGetCommand(args: List<String>): Pair<Command, Int> {
    return getSubCommand(args) ?: throw CommandException(
    translation(
    ...
    private fun validateCommandParameters(command: Command, args: List<String>, idx: Int) {
    ...
    private fun parseCommandParameters(command: Command, args: List<String>, idx: Int): Array<Any?> {
